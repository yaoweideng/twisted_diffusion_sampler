...
def update_via_matrix(R, q, update_mask=None):
    if update_mask is None: update_mask = torch.ones_like(q[..., 0])

    # Compute normalized quaternion
    q_normalized = torch.ones(q.shape[:-1] + (4,), device=q.device, dtype=q.dtype)
    q_normalized[..., 1:] = q*update_mask
    q_normalized = q_normalized / torch.norm(q_normalized, dim=-1, keepdim=True)

    # Compute angle associated with rotation
    angle = 2 * torch.acos(q_normalized[..., 0])
    axis = q/torch.norm(q, dim=-1, keepdim=True)
    
    # Udate rotation matrix via right multiplication of delta rotation
    rot_vec = angle[..., None] * axis
